...
for i, x in enumerate(["ID", "Title", "Author", "Year", "Category"]):
    tk.Label(books, text=x, bg="lightblue",
             font=("Arial", 10, "bold")).grid(row=0, column=i, padx=8, pady=5)
...
for x in ["ID", "Name", "Phone", "Email"]:
    mt.heading(x, text=x)
    mt.column(x, width=200)

mt.grid(row=4, column=0, columnspan=4, padx=15, pady=15)
...
tk.Label(borrow, text="Member ID", bg="lightblue",
         font=("Arial", 10, "bold")).grid(row=0, column=0, padx=15, pady=15)
mid = tk.Entry(borrow, width=25)
mid.grid(row=0, column=1)

tk.Label(borrow, text="Book ID", bg="lightblue",
         font=("Arial", 10, "bold")).grid(row=1, column=0, padx=15, pady=15)
...
tk.Button(borrow, text="Borrow Book", width=15,
          command=borrow_book).grid(row=2, column=0, padx=10, pady=15)
tk.Button(borrow, text="Return Book", width=15,
          command=return_book).grid(row=2, column=1, padx=10, pady=15)
